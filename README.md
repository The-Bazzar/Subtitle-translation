...
正常重启时，download 返回原片后，如果既有 `<name>.mkv` 与当前 `<name>.asr.json` fingerprint 匹配，batch 会跳过 prepare，避免无意义重编码改变 fingerprint 并破坏 recovery；原片比编辑版更新、模型/语言/options 变化或缓存损坏时仍会重新 prepare。新的 alignment generation 提交前必须先删除旧 `<name>.beautified.json`，删除失败会阻止 commit 并写入 cleanup diagnostic，防止新 `<name>.json` 混用旧翻译缓存。alignment 成功后删除 WAV；alignment 失败、取消或 worker crash 时保留 WAV 供诊断和恢复。
