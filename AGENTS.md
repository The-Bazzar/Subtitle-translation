...
- 联网搜索默认由 glossary agent 在同一个 ChatSession 中通过搜索 tool calls 发起；运行时按 `WEB_SEARCH_PROVIDER` 路由 Tavily 或 Exa，并把结果回喂同一 session
...
`glossary.md` 是完整常驻的全局硬规则；retrieved_context 只能补充。`web_evidence:*` 由 sidecar 中规范化的 Tavily/Exa 结果构建，保留 provider、query、域名、标题、URL 和摘要。
