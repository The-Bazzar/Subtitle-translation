...
class BatchMigrationDocumentationTests(unittest.TestCase):
    def test_generated_batch_reports_and_failure_logs_are_gitignored(self):
        ignore_rules = read_script(".gitignore").splitlines()

        for pattern in ("batch-result.json", "batch-worker-failure-*.log"):
            with self.subTest(pattern=pattern):
                self.assertIn(pattern, ignore_rules)

        for broad_pattern in ("*.json", "*.log"):
            with self.subTest(broad_pattern=broad_pattern):
                self.assertNotIn(broad_pattern, ignore_rules)
