...
class AsrCacheTests(unittest.TestCase):
    ...
    def test_artifact_cache_rejects_reextracted_wav_snapshot(self):
        fingerprint = self.fingerprint()
        self.write_cache(fingerprint, self.asr_result())
        self.wav_path.write_bytes(b"reextracted-wav")
...
class WorkerProtocolTests(unittest.TestCase):
    ...
    def test_worker_uses_configured_source_language_without_metadata(self):
        config = dataclasses.replace(self.config, source_language="fra")
        edit_video, wav_path = self.media_pair("configured", language=None)
        ...
        with controller:
            controller.load_asr()
            result = controller.transcribe(self.artifact_for(wav_path))
        ...
        self.assertIsNotNone(
            read_valid_asr_cache(
                edit_video,
                fingerprint,
                self.artifact_for(wav_path).media_generation,
            )
        )
