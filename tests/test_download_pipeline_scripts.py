...
class ScriptBehaviorMixin:
    ...
    def run_prepare(self, *args, ffmpeg_exit=0, create_output=True, nvenc_available=False):
...
function Invoke-FakeFfmpeg {
    ...
    if ($env:FAKE_FFMPEG_CREATE_OUTPUT -eq '1') {
        [System.IO.File]::WriteAllText($Arguments[-1], 'edit')
    }
...
"""
        ...
        env["FAKE_FFMPEG_CREATE_OUTPUT"] = "1" if create_output else "0"
        return subprocess.run(
            [
                PWSH,
                "-NoProfile",
...
printf original > "$output_path"
    ...
    def write_fake_ffmpeg(self, exit_code, create_output, nvenc_available=False):
        output_block = 'printf edit > "${@: -1}"' if create_output else ":"
...
exit {exit_code}
    ...
    def run_prepare(self, *args, ffmpeg_exit=0, create_output=True, nvenc_available=False):
        self.write_fake_ffmpeg(ffmpeg_exit, create_output, nvenc_available)
        self.write_env()
