...
class ProofreadContinuityAndUncertaintyTests(unittest.TestCase):
    ...
    def test_split_events_receive_their_complete_sentence_context(self):
        ...
        for current_index, item in enumerate(items):
            # `sentence_context` is deliberately distinct from neighbor context:
    ...
    def test_naturalness_improvement_does_not_need_a_preservation_checklist(self):
        """Target quality judgment belongs to the model unless a clear regression exists."""
    ...
    def test_clear_accuracy_fix_is_accepted_without_preservation_checklist(self):
        """The regression guard must not turn proofreading into a no-op."""
        source, target, review = t.apply_conservative_proofread_result(
            "He did not agree.",
            "他同意了。",
            "He did not agree.",
        ...
        self.assertEqual(source, "He did not agree.")
        self.assertEqual(target, "他不同意。")
        self.assertEqual(review, {})
    ...
    def test_style_edit_cannot_drop_source_backed_exclusivity(self):
        ...
        self.assertEqual(source, "Only he could open it.")
        self.assertEqual(target, "只有他能打开。")
    ...
    def test_enhanced_concurrency_uses_isolated_search_runtimes_and_merges_in_order(self):
        ...
        self.assertEqual(len(set(runtime_ids)), 2)
