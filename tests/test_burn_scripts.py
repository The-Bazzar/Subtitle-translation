...
class BurnStageTests(unittest.TestCase):
    def test_wrappers_select_the_expected_backend(self):
        self.assertIn("-m subtitle_translation burn --backend ffmpeg", (ROOT / "ffmpeg-burn.ps1").read_text())
        self.assertIn("-m subtitle_translation burn --backend mpv", (ROOT / "mpv-burn.ps1").read_text())
