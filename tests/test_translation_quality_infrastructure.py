...
class TranslationQualityInfrastructureTests(unittest.TestCase):
    ...
    def test_proofread_source_change_preserves_first_original_en(self):
        class FakeBatchLLM:
            provider = "fake"
            batch_size = 1
        ...
        event = t.SplitEvent(0.0, 1.0, "ASR source", "原译文")
        ...
        responses = iter(["corrected source", "second correction"])
        ...
        with patch.object(t, "llm_numbered_batch", side_effect=fake_batch):
            first_changed = t.proofread_split_events(
                transcript,
            ...
            second_changed = t.proofread_split_events(
                transcript,
                t.TranscriptContext.from_json("video.json", "", "en", "zh"),
                FakeBatchLLM(),
                "system",
