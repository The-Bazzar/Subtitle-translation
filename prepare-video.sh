...
run_edit_reencode() {
    ...
    run_reencode_attempt() {
        ...
        local accepted=0
        if [ "$ffmpeg_exit" -eq 0 ] && [ -f "$temporary_path" ]; then
            accepted=1
        elif [ "$label" = "h264_nvenc" ] && [ -s "$temporary_path" ]; then
...
ORIGINAL_VIDEO_ABS="$(resolve_abs_path "$ORIGINAL_VIDEO")"
...
if [ "$ORIGINAL_VIDEO_ABS" = "$EDIT_VIDEO_ABS" ]; then
...
cleanup_temp_edit_video() {
    if [ -n "${TEMP_EDIT_VIDEO:-}" ] && [ -e "$TEMP_EDIT_VIDEO" ]; then
        rm -f -- "$TEMP_EDIT_VIDEO" || echo "Warning: could not clean temporary edit video '$TEMP_EDIT_VIDEO'" >&2
    fi
}
