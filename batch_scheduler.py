    ...
    def _run_asr_wave(self) -> None:
        ...
        for task in self.tasks:
            ...
            try:
                ...
                cached_result = read_valid_asr_cache(task.edit_video_path, fingerprint)
        ...
        if not uncached_tasks:
            cached_tasks = [
                task
                for task in self.tasks
                if task.state is TaskState.SUCCEEDED and task.stage == "asr_ready"
            ]
            if cached_tasks:
                try:
                    with self.worker_factory(self.asr_config):
                        pass
                except Exception as exc:
                    cached_tasks[-1].start_next_phase("asr_worker")
                    cached_tasks[-1].fail(stage="asr_worker", detail=str(exc))
            return

        try:
            with self.worker_factory(self.asr_config) as worker:
                try:
                    load_result = worker.load_asr()
                ...
                for index, (task, fingerprint) in enumerate(uncached_tasks):
                    task.advance("asr")
                    try:
                        result = worker.transcribe(task.wav_path)
                    except (WorkerExitedError, WorkerUnresponsiveError) as exc:
                ...
                if not worker_failed:
                    try:
                        unload_result = worker.unload_asr()
                    except (WorkerExitedError, WorkerUnresponsiveError) as exc:
