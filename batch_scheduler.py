...
class BatchControl:
    ...
    def schedule_cleanup(self, callback: Callable[[], object]) -> None:
        self._schedule_force_callback(callback)
...
class _AlignmentCommitState:
    ...
    def committed(self) -> bool:
        with self._lock:
            return self._phase is _AlignmentCommitPhase.COMMITTED
...
class BatchTask:
    ...
    continue_postprocess_after_interrupt: bool = False
...
class AcquisitionScheduler:
    def __init__(
        ...
        self._file_lock_cancel_events: set[threading.Event] = set()
        self._file_lock_cancel_events_lock = threading.Lock()
    ...
    async def _media_file_transaction(
        ...
        *,
        ...
        with self._file_lock_cancel_events_lock:
            self._file_lock_cancel_events.add(cancel_event)
            if cancel_on_interrupt and self.control.interrupted:
                cancel_event.set()
        ...
        try:
            ...
            try:
                ...
                with self._file_lock_cancel_events_lock:
                    self._file_lock_cancel_events.discard(cancel_event)

    def request_interrupt(self) -> int:
        count = self.control.request_interrupt()
        with self._file_lock_cancel_events_lock:
            for cancel_event in self._file_lock_cancel_events:
                cancel_event.set()
        commit_state = self._active_alignment_commit
        if commit_state is not None:
            decision = commit_state.try_request_cancel()
            if decision is _AlignmentCancelDecision.CANCEL_WON:
                self.control.schedule_cleanup(self._force_abort_worker)
        return count
    ...
    async def _run_worker_pipeline(self) -> None:
        ...
        try:
            ...
            worker_released, release_cancellation = await _await_uninterruptibly(
                release_task
            )
            if not worker_released:
                self._fail_postprocess_for_live_worker(postprocess_jobs)
            ...
            for task in group:
                ...
                if not self.control.worker_admission_open:
                    if commit_state.committed:
                        task.continue_postprocess_after_interrupt = True
                    else:
    ...
    async def _run_postprocess(self, task: BatchTask) -> None:
        ...
        continue_after_interrupt = task.continue_postprocess_after_interrupt
        try:
            task.advance("postprocess")
            task.beautified_candidate_path = self._new_beautified_candidate_path(task)
            async with self._media_transaction(task.edit_video_path):
                async with self._media_file_transaction(
                    task.edit_video_path,
                    cancel_on_interrupt=not continue_after_interrupt,
                ):
                    self._validate_postprocess_generation(task)
                    async with self._cpu_io_slots:
                        if (
                            not self.control.stage_advancement_open
                            and not continue_after_interrupt
                        ):
    ...
    async def _release_worker(self, worker: AsrWorkerController) -> bool:
        try:
        ...
        if worker.is_alive:
            self._record_release_failure(
                "worker_reap",
                RuntimeError("worker is still alive after shutdown and close"),
            )
            return False
        self.worker_released.set()
        return True

    def _fail_postprocess_for_live_worker(
        self,
        postprocess_jobs: Sequence[asyncio.Task[None]],
    ) -> None:
        detail = "worker is still alive after shutdown and close"
        for task in self.tasks:
            if task.state is TaskState.RUNNING and task.stage in {
                "postprocess",
                "burn_waiting",
            }:
                task.fail(stage="worker_release", detail=detail)
        for job in postprocess_jobs:
            if not job.done():
                job.cancel()
    ...
    def _reuse_cached_wav_unlocked(
        self,
        prepared_state: PreparedMediaState,
    ) -> WavArtifact | None:
        source_language = resolve_source_language(
            prepared_state.edit_snapshot.path,
            fallback=self.asr_config.source_language,
        )
        fingerprint = build_asr_fingerprint_from_snapshot(
            prepared_state.edit_snapshot,
            model=self.asr_config.model,
            compute_type=self.asr_config.compute_type,
            source_language=source_language,
            asr_options=self.asr_config.options_dict(),
        )
        cached_entry = read_valid_asr_cache_unlocked(
            prepared_state.edit_snapshot.path,
            fingerprint,
        ...
        if cached_entry is None:
            return None
        try:
            artifact = bind_wav_artifact_unlocked(
                prepared_state.edit_snapshot.path,
                cached_entry.wav_snapshot.path,
                prepared_state.generation,
            )
        except (OSError, MediaGenerationMismatch):
            return None
        if artifact.wav_snapshot != cached_entry.wav_snapshot:
            return None
        return artifact
            ...
            if prepared_state is not None:
                async with self._media_transaction(task.edit_video_path):
                    async with self._media_file_transaction(task.edit_video_path):
                        ...
                        cached_artifact = self._reuse_cached_wav_unlocked(
                            prepared_state
                        )
                        if cached_artifact is not None:
                            task.wav_artifact = cached_artifact
                            task.wav_path = Path(cached_artifact.wav_snapshot.path)
                        else:
                            async with self._cpu_io_slots:
                                if not self.control.stage_advancement_open:
                                    raise StageAdvancementStopped(
