#!/bin/bash
# =============================================================================
# pipeline.sh — 一键流水线: 下载 → 字幕 → 美化 → 术语库 → 翻译 → 硬压
#
# 串联 download.sh → whisper.sh → beautify_srt.py → glossary_builder.py → translate_srt.py → (ffmpeg-burn.sh)
# 从 YouTube 链接直达双语 .zh-en.ass 字幕 / burned.mkv 硬字幕。
#
# 成果物链:
#   VIDEO_PATH → BEAUTIFIED_SRT → glossary.md → ASS_PATH → burned.mkv
#
# 用法:
...
流程:
  ...
  2. WhisperX large-v3-turbo 生成英文字幕 (.srt + .json)
  3. ffmpeg/ffprobe 场景检测 → 字幕时间码吸附对齐 → .beautified.srt
  4. glossary_builder.py 联网搜索 + LLM → 术语知识库 glossary.md
  5. LLM 长句拆分 + 翻译 + 双语校对 → .zh-en.ass + .proofread.srt
  6. ffmpeg 硬压字幕 → burned.mkv (默认启用, BURN=0 跳过)

成果物链: VIDEO_PATH → BEAUTIFIED_SRT → glossary.md → ASS_PATH → burned.mkv
...
示例:
  ./pipeline.sh "https://www.youtube.com/watch?v=xxxxx"
  ./pipeline.sh "https://www.youtube.com/watch?v=xxxxx" -- --preview
  ./pipeline.sh "https://www.youtube.com/watch?v=xxxxx" -- --backup --scene-threshold 0.1
  ./pipeline.sh "https://www.youtube.com/watch?v=xxxxx" -- --aggressive  # 激进模式
  TRANSLATE_PROVIDER=deepseek ./pipeline.sh "url"
  BURN=0 ./pipeline.sh "url"   # 跳过硬压

beautify 选项 (在 -- 之后, 默认值遵循 Subtitle Edit 兼容):
...
if [ "${SKIP_WHISPER:-0}" = "1" ]; then
    ...
    echo "============================================="
    echo "原始字幕已存在, 跳过 — $SRT_PATH"
    echo "============================================="
else
    echo ""
    echo "============================================="
    echo "pipeline — 步骤 2/6: WhisperX 生成字幕"
    echo "============================================="
    echo ""
...
if [ -n "${EXISTING_SRT:-}" ] && [ -f "$EXISTING_SRT" ]; then
    ...
    echo "============================================="
    echo "pipeline — 步骤 3/6: 字幕时间码美化 → .beautified.srt"
    echo "============================================="
    echo ""
...
# 确定给 knowledge/translate 的 SRT 输入
...
KNOWLEDGE_SCRIPT="$SCRIPT_DIR/glossary_builder.py"
...
if [ "${SKIP_KNOWLEDGE:-0}" = "1" ]; then
    ...
    echo "============================================="
    echo ""
    python "$KNOWLEDGE_SCRIPT" "$TRANSLATE_SRC"
    echo ""
fi
...
if [ "${SKIP_TRANSLATE:-0}" = "1" ]; then
    ...
    echo "============================================="
    echo "pipeline — 步骤 5/6: LLM 翻译 (英→中) → .zh-en.ass"
    echo "============================================="
    echo ""
