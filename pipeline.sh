...
#   ./pipeline.sh "https://www.youtube.com/watch?v=xxxxx" -- --no-extend
#
# 环境变量:
...
示例:
  ...
  ./pipeline.sh "https://www.youtube.com/watch?v=xxxxx" -- --scene-threshold 0.25 --snap-distance 0.25
  ./pipeline.sh "https://www.youtube.com/watch?v=xxxxx" -- --no-extend --min-duration 0.3

beautify 选项 (在 -- 之后):
  --scene-threshold N    场景检测灵敏度 (默认 0.3)
